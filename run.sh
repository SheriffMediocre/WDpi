...
while IFS='' read -r line || [[ -n "$line" ]]; do
	...
	then
		...
		for ((  i=0; i<${#info}; i++  )); do
			...
			if [ "$kbcode" != "" ]
			then
				echo "$kbcode" | $installpath/hid-gadget-test $kb > /dev/null
			fi
		done
	elif [ "$cmd" == "ENTER" ] 
	then
		last_cmd="enter"
		echo "$last_cmd" | $installpath/hid-gadget-test $kb > /dev/null
	
	elif [ "$cmd" == "DELAY" ] 
	then
		last_cmd="UNS"
		((info = info*1000))
		$installpath/usleep $info
